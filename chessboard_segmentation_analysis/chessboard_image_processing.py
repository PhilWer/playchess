...
class ImageProcessing():
    ...
    def preprocessing(self, img, gaussian_kernel_size=5):
        ...
        if self.debug:
            cv2.imshow('Preprocessing', np.concatenate(
                (img_gray, img_thresholded), axis=1))
            cv2.waitKey(2000)
    ...
    def chessboard_contour(self, img, preprocessed_img):
        ...
        if self.debug:
            cv2.drawContours(
                img_contours, [largest, chessboard_edge_poly], -1, color, 2)
        ...
        # blank image with same shape of the original one
        img_masked = np.zeros_like(img)
        # fill the ROI with values from original image
        ...
        return img_masked, img_contours
    ...
    def find_intersections(self, horizontals, verticals, image):
        ...
        if self.debug:
            print("\nWe have found: " +
                  str(len(intersections)) + " intersections.\n")
    ...
    def make_squares(self, corners,  img, playing_white):
        ...
        img_centers = self.img_masked.copy()
        # for row in corners:
    ...
    def tune_hough_params(self, img, img_edge):
        ...
        while not bo.max['target'] == 0:            # check convergence to optimum
            if iter >= HOUGH_TUNING_ITERS_LIMIT:    # check no. of iterations
                print('The limit of ' + str(HOUGH_TUNING_ITERS_LIMIT) +
                      ' has been reached and the optimum has not been found...')
                break
            # Run 10 iterations of Bayesian Optimization
    ...
    def segmentation_sequence(self, img, tiago_color='white'):
        ...
        # Dilate the edges
        img_dilated_edge = self.dilation(
            img_edge, kernel_size=(5, 5), iterations=5)
        # Chessboard edges used to eliminate unwanted point in assign intersections function
        img_contour_masked, img_contour_edge = self.chessboard_contour(
            img, img_dilated_edge)
        ...
        if self.hough_autotune:  # the parameters of the Hough transform will be automatically
            ...
            corners, intersection_sorted = self.tune_hough_params(
                img, img_edge_masked)
        else:
            # Run the hough lines
            ...
            corners, intersection_sorted = self.track_bar(
                img, img_edge_masked, corners)
        ...
        with open(os.path.join(tmp_yaml_path, 'store_chess_board_edges.yaml'), 'w') as file:
            documents = yaml.dump(chessboard_vertices, file)

        with open(os.path.join(tmp_yaml_path, 'corners_names_empty_square.yaml'), 'w') as file:
...
if __name__ == "__main__":
    ...
    # Set it to True to display the outcome of each processing step.
    DEBUG = False
    SAVE = True    # Set it to True to save the outcomes into yaml files.
    ...
    image = cv2.imread(
        '/home/pal/tiago_public_ws/src/playchess/config/tmp/rgb/rgb_image_for_plane_model.png')
    ...
    image_processing = ImageProcessing(
        debug=DEBUG, save=SAVE, hough_autotune=True)
    # Run the chessboard segmentation and cell's centers identification
    __, img_out_seg, img_in_seg, __, __ = image_processing.segmentation_sequence(
        image)
    # Display the results
