...
class PclProcessing:
    ...
    def get_points_in_matrix_from_pointstamped(self, pointsStamped):
        ...
        points_matrix = np.zeros((points_number, 3))
        for i in range(0, points_number):
            points_matrix[i] = [pointsStamped[i].point.x,
                                pointsStamped[i].point.y, pointsStamped[i].point.z]
    ...
    def create_pointcloud_from_depth(self):
        ...
        for u in range(640):
            ...
            for v in range(480):
                row = v
                # Save the points' coordinates, depth information and color if the depth value is not 'nan'
        ...
        for element in possible_squares:
            ...
            if points_number > threshold:  # The square is occupied.
                ...
                if self.live_chessboard_situation_complete[current_square][1] == 'none':
                    ...
                    ending_square = current_square
                    # Update the live_chessboard_situation
                    self.live_chessboard_situation_complete[current_square][
                        0] = self.live_chessboard_situation_complete[square][0]
                    self.live_chessboard_situation_complete[current_square][1] = self.opposite_color
                    self.live_chessboard_situation_complete[square][0] = 'none'
                    self.live_chessboard_situation_complete[square][1] = 'none'
                    self.found = True
    ...
    def pawn_checking(self, square, chessboard, threshold, squares_boundboxes, color_threshold, is_color_over):
        ...
        if not self.found:  # If the move has not been identified yet, a capture has happened.
            ...
            if capture_square_2 in rows_indexes[next_row] and capture_square_1 in rows_indexes[next_row]:
                ...
                if points_number_2 > threshold and points_number_1 < threshold:
                    # Save the name of the current square
                    for key, value in self.squares_to_index:
                        if value == capture_square_2:
                            ending_square = key
                ...
                if points_number_1 > threshold and points_number_2 < threshold:
                    # Save the name of the current square
                    for key, value in self.squares_to_index:
                        if value == capture_square_1:
                            ending_square = key
    ...
    def query_promoted_piece(self, question, default="queen"):
        ...
        valid = {"queen": 'queen', "q": 'queen', "rook": 'rook', "r": 'rook',
                 "knight": 'knight', "k": 'knight', "bishop": 'bishop', "b": 'bishop', }
        if default is None:
        ...
        while True:
            ...
            if default is not None and choice == "":
                ...
                sys.stdout.write(
                    "Please respond with 'queen' or 'rook' or 'knight' or 'bishop' " "(or 'q', 'r', 'k', 'b').\n")
    ...
    def update_situation(self, en_passant, en_passant_square, castle, castle_squares, promotion, ending_square, start_square):
        ...
        if en_passant:
            ...
            self.live_chessboard_situation_complete[ending_square][
                0] = self.live_chessboard_situation_complete[start_square][0]
            ...
            if castle_squares[0] == 'A8' or castle_squares[0] == 'A1':
                # rook
                self.live_chessboard_situation_complete[self.castle_ending_squares[0]
                                                        ][0] = self.castle_squares[4]
                self.live_chessboard_situation_complete[self.castle_ending_squares[0]
                                                        ][1] = self.opposite_color
                # king
                self.live_chessboard_situation_complete[self.castle_ending_squares[1]
                                                        ][0] = self.castle_squares[3]
                self.live_chessboard_situation_complete[self.castle_ending_squares[2]
                                                        ][1] = self.opposite_color
                ...
                # rook
                self.live_chessboard_situation_complete[self.castle_ending_squares[3]
                                                        ][0] = self.castle_squares[5]
                self.live_chessboard_situation_complete[self.castle_ending_squares[3]
                                                        ][1] = self.opposite_color
                # king
                self.live_chessboard_situation_complete[self.castle_ending_squares[2]
            ...
            promoted_piece = self.query_promoted_piece(
                'What piece have been promoted?')
    ...
    def calibrate_points(self):
        ...
        # PointCloud with iformation for calibration.
        ...
        starting_squares_matrix = self.get_points_in_matrix_from_array(
            self.squares_centers)
        # To use if I'm usign the original points in the eyes reference frame.
        starting_vertices_matrix = self.get_points_in_matrix_from_array(
            self.vertices)
    ...
    def look_for_pieces(self, pcl, chessboard, squares_boundboxes, threshold, color_threshold, is_color_over):
        ...
        while checking_flag:
            for occupied in self.currently_occupied:
                ...
                for key, values in self.columns_indexes.items():
                    if index in values:
                        column = key
                # If the points over the square are less than a threshold, it means that the piece have been moved. take the 30% of the threshold to exclude the pieces covered by pieces in front of them.
                if points_number < (threshold * 0.3):
        ...
        # Update the live_chessboard_situation
        self.update_situation(en_passant, en_passant_square, castle,
                              castle_squares, promotion, ending_square, move_square)
...
def main():
    ...
    # To use if I'm using the original points in the eyes reference frame.
    squares_matrix = pcl_processor.get_points_in_matrix_from_array(
        pcl_processor.squares_centers)
    # To use if I'm usign the original points in the eyes reference frame.
    vertices_matrix = pcl_processor.get_points_in_matrix_from_array(
        pcl_processor.vertices)
    # Creation of a red PointCloud of the squares centers computed segmenting the chessboard.
    squares_pointcloud = pcl_processor.create_pointcloud(
        squares_matrix, [1, 0, 0])
    # Creation of a red PointCloud of the vertices computed segmenting the chessboard.
    vertices_pointcloud = pcl_processor.create_pointcloud(
    ...
    # Rotation matrix
    R, reference_normal = pcl_processor.get_rot_matrix(
        pcl, normals, bottom_left_vertex)

    # Create a bounding box around the chessboard
    bounding_box_chessboard, frame = pcl_processor.create_bounding_box(
    ...
    sum_length = 0
    for i in range(1, 7):
        sum_width = sum_width + abs(squares_pointcloud.points[pcl_processor.rows_indexes['row8'][i]]
                                    [0] - squares_pointcloud.points[pcl_processor.rows_indexes['row8'][i-1]][0])
        sum_width = sum_width + abs(squares_pointcloud.points[pcl_processor.rows_indexes['row5'][i]]
                                    [0] - squares_pointcloud.points[pcl_processor.rows_indexes['row5'][i-1]][0])
        sum_width = sum_width + abs(squares_pointcloud.points[pcl_processor.rows_indexes['row1'][i]]
                                    [0] - squares_pointcloud.points[pcl_processor.rows_indexes['row1'][i-1]][0])
        sum_length = sum_length + abs(squares_pointcloud.points[pcl_processor.columns_indexes['columnA'][i]]
                                      [1] - squares_pointcloud.points[pcl_processor.columns_indexes['columnA'][i-1]][1])
        sum_length = sum_length + abs(squares_pointcloud.points[pcl_processor.columns_indexes['columnD'][i]]
                                      [1] - squares_pointcloud.points[pcl_processor.columns_indexes['columnD'][i-1]][1])
        sum_length = sum_length + abs(squares_pointcloud.points[pcl_processor.columns_indexes['columnH'][i]]
                                      [1] - squares_pointcloud.points[pcl_processor.columns_indexes['columnH'][i-1]][1])
    ...
    for squares in squares_pointcloud.points:
        bounding_box_square, frame = pcl_processor.create_bounding_box(
    ...
    color_threshold = [((color_RGB[0] + opponent_color_RGB[0]) / 2), ((color_RGB[1] +
                                                                       opponent_color_RGB[1]) / 2), ((color_RGB[2] + opponent_color_RGB[2]) / 2)]
    ...
    # Look for pieces
    moved_piece, ending_square = pcl_processor.look_for_pieces(
