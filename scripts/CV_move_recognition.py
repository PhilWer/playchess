...
PLAYCHESS_PKG_DIR = os.path.normpath(
...
class DepthProcessing:
    ...
    def move_identification(self, move):
        ...
        if en_passant:
            ...
            print(
                'self.live_chessboard_situation[en_pasent_square]', self.possible_enpasent,)
            for i in self.possible_enpasent:
    ...
    def identify_moved_piece(self, move):
        ...
        with open(os.path.join(mv_folder, 'recognized_move.txt'), "w") as file:
            ...
            file.write("time_taken_for_move_recognition:{}".format(
                str(elapsed_time)))
