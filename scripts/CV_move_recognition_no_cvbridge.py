...
from vision_utils import decoder
...
class DepthProcessing:
    ...
    def _convert_image_msg(self, msg, rgb=True):
        if rgb:
            encoding = "bgr8"
            dtype = np.int32
            img = decoder.decode_Image_RGB(msg)
        else:   # if depth
            encoding = "32FC1"
            dtype = np.float32
            img, __ = decoder.decode_Image_depth(msg, depth_in_m = False)        

        if img is None:
            rospy.loginfo('Converting the following msg resulted in None:\n{}'.format(msg))
            
        # Converting to np array (needed?)
        np_arr = np.array(img, dtype=dtype)
        ''' Commented to remove the CvBridge dependency
        try:
