...
def CallbackPcSaved(data):
    if data:
        rospy.loginfo(
            "Initial Pointcloud saved in the playchess/PointcloudForCalibration folder.")
...
# Parameters loading
z_coord_chessboard_mean_file = rospy.get_param(
    '/playchess/z_coord_chessboard_mean')
...
class Playing:
    ...
    def chess_move(self, start_place, end_place, centroids, clock_pose, box_pose, z_coord_chessboard):
        ...
        # 1. if the error comes from straight_eef_movement (i.e. highering or
        which_error = 1
        ...
        try:
            ...
            if start_square == 'short' or start_square == 'long':  # Castle happened.
                ...
                self.motions.playmotion_movement(
                    'open_gripper', planning=False)
                which_error = 1
                # Lower the gripper
                ...
                # Save the piece that is going to be moved.
                piece = self.live_chessboard_situation[start_place][0]
                # Save the piece that is going to be taken (if any).
                taken_piece = self.live_chessboard_situation[end_place][0]
                ...
                # QUANDO DOVREBBE VERIFICARSI CHE END_PLACE SIA BOX?
                if end_place in last_squares or end_place == 'box':
                ...
                # If the end target place is free from pieces, it's a simple move or an en-passant move.
                if taken_piece == 'none':
                    # Implement the capture of the pawn if en-passant happened.
                    if en_passant_square != 'none':
                        self.tilt = 'none'
                        # Save the name of the pawn that is captured.
                        taken_piece = self.live_chessboard_situation[en_passant_square][0]
                        self.motions.gripper_over_piece(
                            en_passant_square, centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                        self.motions.playmotion_movement(
                            'open_gripper', planning=False)
                        which_error = 1
                        fraction, translation = self.motions.straight_eef_movement(
                            self.motions.arm, 'z', -offset_start_place, avoid_collisions=False)  # Lower the gripper
                        # Close the gripper to pick the piece.
                        self.motions.pick_piece(
                            taken_piece, self.pieces_coordinates)
                        fraction, translation = self.motions.straight_eef_movement(
                            self.motions.arm, 'z', offset_start_place, avoid_collisions=False)  # Higher the gripper
                        ...
                        self.motions.gripper_over_box(
                            box_pose, z_coord_chessboard)
                        # Place the piece in the end square.
                        self.motions.place_piece('box')
                        # Go back to the old variable, to avoid going in the other loop.
                        taken_piece = 'none'
                        en_passant_square = 'none'  # Change back the flag

                    if start_place in last_squares:
                        self.motions.gripper_over_piece_last_row(
                            start_place, centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard, False)
                        self.motions.playmotion_movement(
                            'open_gripper', planning=False)
                        self.tilt = 'avanti'
                    elif start_place in first_squares and end_place in last_squares:
                        self.tilt = 'indietro'
                        self.motions.gripper_over_piece_special(
                            start_place, centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                        self.motions.playmotion_movement(
                            'open_gripper', planning=False)
                    ...
                    if not exception_2_flag and not exception_1_flag and not exception_3_flag:
                        which_error = 10
                        # Move the arm with straight movements over the chessboard.
                        self.motions.straight_movements_chessboard(
                            self.motions.arm, centroids, end_place, squares_to_index, self.tilt, clock_pose, box_pose, avoid_collisions=False)
                    elif exception_1_flag:
                        # Finish to move the arm.
                        self.motions.gripper_over_piece_last_row(
                            end_place, centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard, True)
                    elif exception_2_flag:
                        # Finish to move the arm.
                        self.motions.gripper_over_piece_special(
                            end_place, centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    elif exception_3_flag:
                        # Finish to move the arm.
                        self.motions.gripper_over_piece_last_row(
                            end_place, centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard, False)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece(end_place)
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    self.motions.gripper_over_clock(
                    ...
                    #self.motions.execute_remembered_joint_values(self.motions.arm, 'arm_over_box')
                    # Place the taken piece into the box.
                    self.motions.place_piece('box')
                    ...
                    if start_place in last_squares and end_place not in first_squares:
                        ...
                        self.motions.gripper_over_piece_last_row(
                            start_place, centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard, True)
                        self.motions.playmotion_movement(
                            'open_gripper', planning=False)
                        self.tilt = 'avanti'
                    elif start_place in first_squares and end_place in last_squares:
            ...
            rospy.loginfo(
                'The straight movement startegy was not successful. Try planning the movement.')
            ...
            if which_error != 1:  # TODO: error dictionary to display meaningful strings
                if which_error == 2:
                    self.motions.gripper_over_piece(
                        'g1', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece('g1')
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    # Move the rook
                    piece = 'rook_h1'
                    self.motions.gripper_over_piece(
                        'h1', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    self.motions.playmotion_movement(
                        'open_gripper', planning=False)
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_start_place, avoid_collisions=False)  # Lower the gripper
                    ...
                    # Move the arm with straight movements over the chessboard.
                    self.motions.straight_movements_chessboard(
                        self.motions.arm, centroids, 'f1', squares_to_index, self.tilt, clock_pose, box_pose, avoid_collisions=False)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece('f1')
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    # click the clock
                    self.motions.gripper_over_clock(
                    ...
                    self.motions.gripper_over_piece(
                        'f1', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    ...
                    self.motions.gripper_over_piece(
                        'c1', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece('c1')
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    ...
                    piece = 'rook_a1'
                    self.motions.gripper_over_piece(
                        'a1', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    self.motions.playmotion_movement(
                        'open_gripper', planning=False)
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_start_place, avoid_collisions=False)  # Lower the gripper
                    ...
                    # Move the arm with straight movements over the chessboard.
                    self.motions.straight_movements_chessboard(
                        self.motions.arm, centroids, 'd1', squares_to_index, self.tilt, clock_pose, box_pose, avoid_collisions=False)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece('d1')
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    # Click the clock
                    self.motions.gripper_over_clock(
                    ...
                    self.motions.gripper_over_piece(
                        'd1', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    ...
                    self.motions.gripper_over_piece(
                        'g8', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece('g8')
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    # Move the rook
                    piece = 'rook_h8'
                    self.motions.gripper_over_piece(
                        'h8', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    self.motions.playmotion_movement(
                        'open_gripper', planning=False)
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_start_place, avoid_collisions=False)  # Lower the gripper
                    ...
                    # Move the arm with straight movements over the chessboard.
                    self.motions.straight_movements_chessboard(
                        self.motions.arm, centroids, 'f8', squares_to_index, self.tilt, clock_pose, box_pose, avoid_collisions=False)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece('f8')
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    # click the clock
                    self.motions.gripper_over_clock(
                    ...
                    self.motions.gripper_over_piece(
                        'f8', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    ...
                    self.motions.gripper_over_piece(
                        'c8', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece('c8')
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    # Move the rook
                    piece = 'rook_a8'
                    self.motions.gripper_over_piece(
                        'a8', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    self.motions.playmotion_movement(
                        'open_gripper', planning=False)
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_start_place, avoid_collisions=False)  # Lower the gripper
                    ...
                    # Move the arm with straight movements over the chessboard.
                    self.motions.straight_movements_chessboard(
                        self.motions.arm, centroids, 'd8', squares_to_index, self.tilt, clock_pose, box_pose, avoid_collisions=False)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                    self.motions.place_piece('d8')
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', offset_end_place, avoid_collisions=False)  # Higher the gripper
                    # Click the clock
                    self.motions.gripper_over_clock(
                    ...
                    self.motions.gripper_over_piece(
                        'd8', centroids, self.live_chessboard_situation, squares_to_index, z_coord_chessboard)
                    which_error = 1
                    fraction, translation = self.motions.straight_eef_movement(
                        self.motions.arm, 'z', -offset_end_place, avoid_collisions=False)  # Lower the gripper
                    # Place the piece in the end square.
                ...
                rospy.loginfo(
                    'Go back to state 11, manually change the GUI and ask TIAGo to perform the move another time.')
                chess_move_not_exectuted_yet = True
    ...
    def reset(self, strategy):  # TODO
        ...
        # In case the object is still attached
        self.scene.remove_attached_object(link='gripper_grasping_frame')
        ...
        if strategy == 'nearby':
            ...
            goal_pose = self.motions.arm.get_current_pose(
            ...
            if goal_pose.header.frame_id == 'base_footprint':
                ...
                # QUESTI VALORI VANNO BENE ANCHE PER LA MIA APPLICAZIONE??
                goal_pose.pose.position.z += 0.1
                success, __ = self.motions.eef_to_valid_pose(
                    self.motions.arm, goal_pose.pose.position, goal_pose.pose.orientation, 'z', 0.05, 0.10)
                if success:
                    rospy.loginfo(
                        'The \'nearby\' homing position has been successfully reached.')
                else:
                    rospy.logerr(
                        'Impossible to reach the \'nearby\' homing position.')
            else:
                rospy.logerr('The current pose is in the ' + goal_pose.header.frame_id +
                             '. The \'nearby\' homing is meant to work in the \'base_footprint\' frame, hence it won\'t be executed.')
        elif strategy == 'home':
            # Bring the arm far from the table
            self.motions.execute_remembered_joint_values(
                self.motions.arm, 'open_wide_right')
            # Bring the arm close to the robot body
            # Be careful: this is pre-recorded.
            self.motions.playmotion_movement('home', timeout=20)
        else:
            rospy.logerr(
                'No valid exit strategy was given, the robot is going to stay there.')

        if self.clear_all:
            rospy.loginfo('Clearing the octomap.')
            # keeping the octomap for a certain number of iteration,
            rospy.wait_for_service('/clear_octomap')
            # (in a controlled environment) would spare some time
            clear_octomap = rospy.ServiceProxy('/clear_octomap', Empty)
            clear_octomap()


if __name__ == '__main__':
    if len(sys.argv[:1]) > 0:  # sys.argv[0] is the name of the script
        if sys.argv[1] in ['true', 'True']:
            ready = True
            print('Starting a new game relying on previous initilization')
        elif sys.argv[1] in ['false', 'False']:
            ready = False
            print('Starting a new game from scratch.')
        else:
            raise ValueError('The first command-line argument should be a `True` or `False` string, indicating wheter the initialization has to be run or not.',
                             'You instead passed {}'.format(sys.argv[1]))
        # Check wheter using the joystick or not
        if sys.argv[2] in ['true', 'True']:
            joystick = True
            print('Playing without joystick')
        elif sys.argv[2] in ['false', 'False']:
            joystick = False
            print('Playing without joystick')
        else:
            raise ValueError('The second command-line argument should be a `True` or `False` string, indicating wheter playing with the joystick or not.',
                             'You instead passed {}'.format(sys.argv[1]))
    move = False # flag to decide wheter to play user's moves with the TIAGo's  arm or not
                # TODO. Set from launch
    ...
    rospy.Subscriber('/en_passant_square_to_move',
                     String, CallbackEnPassantSquare)
    ...
    try:
        while not rospy.is_shutdown():
            if state.data == 0:
                if joystick:
                    pass  # wait for the joystick button to be pressed
                else:
                    ans = raw_input('Press Enter to start the game.')
                    state_publisher.publish(1)
            if state.data == 1:  # State of TIAGO initialization.
                grasper = Grasping()
                # Extend torso, look down, inspect surroundings (if necessary).
                grasper.initialize_TIAGo()
                # NB: the state is changed in grasp_piece.py

            elif state.data == 4:  # State of chessboard segmentation.
                if segmentation_not_launched:
                    segmentation_launcher = roslauncher.roslaunch_from_file(
                        PLAYCHESS_PKG_DIR + '/launch/segmentation.launch')
                    segmentation_not_launched = False

            elif state.data == 5:  # State of asking confirmation for the chessboard segmentation
                # Shutdown the segmentation node.
                segmentation_launcher.shutdown()
                segmentation_not_launched = True

                # Launch the CV depth images processing.
                if CV_launcher_not_launched:
                    print(PLAYCHESS_PKG_DIR +
                          '/launch/CV_move_recognition.launch')
                    CV_launcher = roslauncher.roslaunch_from_file(
                        PLAYCHESS_PKG_DIR + '/launch/CV_move_recognition.launch')
                    CV_launcher_not_launched = False

            elif state.data == 7:  # State of search for ARUCO markers.
                if aruco_detector_not_launched:
                    aruco_launcher = roslauncher.roslaunch_from_file(
                        PLAYCHESS_PKG_DIR + '/launch/multi_detector.launch')
                    aruco_detector_not_launched = False
                if pc_saver_not_launched:
                    pc_saver_launcher = roslauncher.roslaunch_from_file(
                        PLAYCHESS_PKG_DIR + '/launch/pc_saver.launch')
                    pc_saver_not_launched = False

            elif state.data == 40:  # State of asking for the markers localization confirmation.
                # Shutdown the markers searcher node.
                aruco_launcher.shutdown()
                aruco_detector_not_launched = True
                ...
                # Prepare TIAGo to grasp pieces.
                escape_strategy = grasper.prepare_TIAGo(
                    clock_marker_pose, z_coord_chessboard_mean, box_marker_pose)
                ...
                if move and chess_move_not_exectuted_yet:
                    ...
                    # Execute the complete move. #TOVERIFY: TIENE TRANSFORMED_CENTRIDS?
                    player.chess_move(start_square, end_square, transformed_centroids,
                                      clock_marker_pose, box_marker_pose, z_coord_chessboard_mean)
                    chess_move_not_exectuted_yet = False
                elif not move:
                    # ans = raw_input('Carry out the move by hand.\nPress Enter when done...')
                    #state_publisher.publish(14) # trigger the move recognition
                    #chess_move_not_exectuted_yet = False
                    pass
                # else:
                #     rospy.logerr('You should not enter this condition! Stop the application and debug your code.')
