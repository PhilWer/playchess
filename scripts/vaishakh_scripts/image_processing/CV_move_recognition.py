...
from scripts.useful_functions.move_detection_ssim_and_pcl import OccupancyChecker
ready = False
...
class DepthProcessing:
    ...
    def CallbackState(self, data):
        ...
        if data.data == 14:
            ...
            self.depth_data = rospy.wait_for_message(
                '/xtion/depth/image_raw', Image)
            ...
            self.depth_arr_before = self.depth_image.astype(
            ...
            self.rgb_img_before = cv2.cvtColor(
                self.rgb_image, cv2.COLOR_BGR2RGB)
            if self.save:
                self.move += 1
                ...
                np.savetxt(mv_folder + '/depth_before.csv',
                           self.depth_arr_before, delimiter=",")
        ...
        if data.data == 15 and analysis_not_done_yet:
            ...
            self.depth_image = np.frombuffer(
                data.data, dtype=np.uint16).reshape(data.height, data.width)
            # converting to float and scaling to 0-1 range
            self.depth_arr_after = self.depth_image.astype(
                np.float32) / self.depth_image_norm_factor
            # save depth data to file
            np.savetxt("empty_depth_image.csv",
                       self.depth_image, delimiter=",")
            ...
            self.rgb_image = np.frombuffer(data.data, dtype=np.uint8).reshape(
                data.height, data.width, -1)
            self.rgb_img_after = cv2.cvtColor(
                self.rgb_image, cv2.COLOR_BGR2RGB)
