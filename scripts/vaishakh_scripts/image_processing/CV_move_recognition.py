...
from move_detection_ssim_and_pcl import OccupancyChecke 
ready = False
...
pointcloud_for_calibration = PLAYCHESS_PKG_DIR + \
    '/PointcloudForCalibration/calibrationCloud.ply'
...
class DepthProcessing:
    ...
    def __init__(self):
        ...
        self.done_publisher = rospy.Publisher(
            '/synchronization_done', Bool, queue_size=10)

        # Subscribers initialization
        self.done_subscriber = rospy.Subscriber(
            "/synchronization_done", Bool, self.identify_moved_piece)
    ...
    def synchronize_rgb_depth(self):
        ...
        self.bridge = CvBridge()
        self.rgb_subscriber_state2 = message_filters.Subscriber(
            '/xtion/rgb/image_rect_color/compressed', CompressedImage)
        # /compressed', CompressedImage)    xtion/depth/image_rect/compressedDepth  PRIMA ERA: /xtion/depth/image e Image
    ...
    def depth_image_callback(self, data):
        try:
            self.depth_subscriber_state2 = self.bridge.imgmsg_to_cv2(data, "32FC1")
            #converting to np array
            self.depth_subscriber_state2 = np.array(self.depth_subscriber_state2, dtype=np.float32)   
    ...
    def king_checking(self, square, chessboard, threshold, squares_boundboxes, color_threshold, is_color_over):
        ...
        if square == 'e8':
            if self.live_chessboard_situation['f8'][0] == 'none' and self.live_chessboard_situation['g8'][0] == 'none' and self.live_chessboard_situation['h8'][0] == 'rook_h8':
                square_box = self.crop(
                    chessboard, squares_boundboxes[self.squares_to_index['f8']])
            ...
            if ending_square == 'none' and self.live_chessboard_situation['d8'][0] == 'none' and self.live_chessboard_situation['c8'][0] == 'none' and self.live_chessboard_situation['b8'][0] == 'none' and self.live_chessboard_situation['a8'] == 'rook_a8':
                square_box = self.crop(
                    chessboard, squares_boundboxes[self.squares_to_index['d8']])
                points_number = len(np.asarray(square_box.points))
                if points_number > threshold:
                    ending_square = 'castle'
        ...
        if square == 'e1':
            if self.live_chessboard_situation['f1'][0] == 'none' and self.live_chessboard_situation['g1'][0] == 'none' and self.live_chessboard_situation['h1'][0] == 'rook_h1':
                square_box = self.crop(
                    chessboard, squares_boundboxes[self.squares_to_index['f1']])
                points_number = len(np.asarray(square_box.points))
                if points_number > threshold:
                    ending_square = 'castle'
            ...
            if ending_square == 'none' and self.live_chessboard_situation['d1'][0] == 'none' and self.live_chessboard_situation['c1'][0] == 'none' and self.live_chessboard_situation['b1'][0] == 'none' and self.live_chessboard_situation['a1'][0] == 'rook_a1':
                square_box = self.crop(
                    chessboard, squares_boundboxes[self.squares_to_index['d1']])
                points_number = len(np.asarray(square_box.points))
                if points_number > threshold:
                    ending_square = 'castle'
        ...
        if ending_square == 'none':
            ...
            for element in possible_squares:
                ...
                if self.verbose:
                    ...
                    print(points_number)
                # The square is occupied and is the one with the higher number of points.
                if points_number > threshold * 1.3 and points_number > higher_points_number and self.live_chessboard_situation[current_square][1] != self.opposite_color:
                    ...
                    piece_color = self.recognize_color(
                        mean_RGB, color_threshold, is_color_over)
                    if self.verbose:
            ...
            if higher_points_square != 'none':
                if self.live_chessboard_situation[higher_points_square][1] == 'none':
                    ...
                    if self.live_chessboard_situation[second_higher_points_square][1] != self.opposite_color and second_piece_color == self.opposite_color:
                        ...
                        # Update the live_chessboard_situation
                        self.live_chessboard_situation[second_higher_points_square][
                            0] = self.live_chessboard_situation[square][0]
        ...
        if not self.found:  # If the end square has not been found again, it means that the pcl processing has not worked properly.
            rospy.loginfo(
                'PCL processing has not worked properly. Try processing again.')
    ...
    def queen_checking(self, square, chessboard, threshold, squares_boundboxes, color_threshold, is_color_over):
        ...
        while count < 8:
            # and count1 <= ciao:
            if (index + 7 * count) in range(64) and not flag1:
                for key, value in self.squares_to_index.items():
                    if value == (index + 7 * count):
                        current_square_diag = key
                if self.live_chessboard_situation[current_square_diag][1] == 'none':
                    possible_squares = np.append(
                        possible_squares, index + 7 * count)
                else:
                    possible_squares = np.append(
                        possible_squares, index + 7 * count)
                    flag1 = True
            # and count2 <= (7-ciao1):
            if (index - 9 * count) in range(64) and not flag2:
                for key, value in self.squares_to_index.items():
                    if value == (index - 9 * count):
                        current_square_diag = key
                if self.live_chessboard_situation[current_square_diag][1] == 'none':
                    possible_squares = np.append(
                        possible_squares, index - 9 * count)
                else:
                    possible_squares = np.append(
                        possible_squares, index - 9 * count)
                    flag2 = True
            # and count3 <= (7 - ciao1):
            if (index - 7 * count) in range(64) and not flag3:
                for key, value in self.squares_to_index.items():
                    if value == (index - 7 * count):
                        current_square_diag = key
                if self.live_chessboard_situation[current_square_diag][1] == 'none':
                    ...
                    flag3 = True
            # and count4 <= (7 - ciao):
            if (index + 9 * count) in range(64) and not flag4:
                for key, value in self.squares_to_index.items():
                    if value == (index + 9 * count):
                        current_square_diag = key
                if self.live_chessboard_situation[current_square_diag][1] == 'none':
    ...
    def knight_checking(self, square, chessboard, threshold, squares_boundboxes, color_threshold, is_color_over):
        ...
        for element in possible_squares:
            ...
            for key, value in self.squares_to_index.items():
                if value == element:
                    current_square = key
                    square_box = self.crop(
                        chessboard, squares_boundboxes[element])
                    points_number = len(np.asarray(square_box.points))
                    if self.verbose:
                        print('++++ curr square: '+str(current_square))
                        print('++++ points: '+str(points_number))
                    # The square is occupied and is the one with the higher number of points.
                    if points_number > threshold * 0.8 and points_number > higher_points_number and self.live_chessboard_situation[current_square][1] != self.opposite_color:
                        RGB_colors = square_box.colors
                        mean_RGB = self.mean_RGB_points([RGB_colors])
                        piece_color = self.recognize_color(
                            mean_RGB, color_threshold, is_color_over)
        ...
        if higher_points_square != 'none':
            ...
            # and piece_color == self.opposite_color:
            elif self.live_chessboard_situation[higher_points_square][1] == self.color:
                # The piece has been captured.
                ending_square = higher_points_square
                self.found = True
        else:
        ...
        if self.verbose:
            rospy.loginfo('checking_knight took ' +
                          str(time() - t_knight) + 's')
    ...
    def bishop_checking(self, square, chessboard, threshold, squares_boundboxes, color_threshold, is_color_over):
        ...
        if prev1_column_ind in range(8):
            if prev1_row_ind in range(8):
                for casella_colonna in self.columns_indexes[columns[prev1_column_ind]]:
                    square = [k for k, v in self.squares_to_index.items()
                              if v == (casella_colonna)]
        ...
        if self.verbose:
            rospy.loginfo('checking_bishop took ' +
                          str(time() - t_bishop) + 's')
    ...
    def pawn_checking(self, square, chessboard, threshold, squares_boundboxes, color_threshold, is_color_over):
        ...
        # Sort the squares of the column in ascending order.
        self.columns_indexes[column].sort()
        # Save the index of the current square of the piece in the column array.
        ind = self.columns_indexes[column].index(index)
        ...
        # If the pawn is in its starting position, it could be moved straight for two steps. Check this possibility.
        if not self.found and index in self.rows_indexes[self.seventh_row]:
            next_square_x2 = self.columns_indexes[column][ind + 2]

            square_box = self.crop(
                chessboard, squares_boundboxes[next_square_x2])
            points_number = len(np.asarray(square_box.points))
            if self.verbose:
            ...
            if points_number > threshold * 0.7 and piece_color == self.opposite_color:
                # Save the name of the current square
                for key, value in self.squares_to_index.items():
        ...
        if not self.found:  # If the move has not been identified yet, a capture has happened.
            ...
            if capture_square_2 in self.rows_indexes[next_row] and capture_square_1 in self.rows_indexes[next_row]:
                ...
                square_box_2 = self.crop(
                    chessboard, squares_boundboxes[capture_square_2])
                square_box_1 = self.crop(
                    chessboard, squares_boundboxes[capture_square_1])
                ...
                piece_color_2 = self.recognize_color(
                    mean_RGB_2, color_threshold, is_color_over)
                piece_color_1 = self.recognize_color(
                    mean_RGB_1, color_threshold, is_color_over)
                if self.verbose:
                ...
                # Square 2 is occupied, square 1 is not.
                if points_number_2 > threshold*0.7 and points_number_1 < threshold*0.7:
                    # Save the name of the current square
                    for key, value in self.squares_to_index.items():
                        if value == capture_square_2:
                            ending_square = key
                ...
                # Square 1 is occupied, square 2 is not.
                if points_number_1 > threshold*0.7 and points_number_2 < threshold*0.7:
                    # Save the name of the current square
                    for key, value in self.squares_to_index.items():
                        if value == capture_square_1:
                            ending_square = key
                ...
                # Both square 1 and 2 are occupied.
                if points_number_2 > threshold*0.7 and points_number_1 > threshold*0.7:
    ...
    def calibrate_points(self):
        ...
        # PointCloud with information for calibration.
        starting_pcl = o3d.io.read_point_cloud(pointcloud_for_calibration)
        ...
        # To use if I'm using the original points in the eyes reference frame.
        starting_squares_matrix = self.get_points_in_matrix_from_pointstamped(
            self.squares_centers)
        # To use if I'm usign the original points in the eyes reference frame.
        starting_vertices_matrix = self.get_points_in_matrix_from_pointstamped(
            self.vertices)
        # Creation of a red PointCloud of the squares centers computed segmenting the chessboard.
        starting_squares_pointcloud = self.create_pointcloud(
            starting_squares_matrix, [1, 0, 0])
        # Creation of a red PointCloud of the vertices computed segmenting the chessboard.
        starting_vertices_pointcloud = self.create_pointcloud(
            starting_vertices_matrix, [0, 1, 0])
        # Normals estimation
        ...
        starting_center = [(starting_e4[0] + starting_d5[0])/2, (starting_d5[1] +
                                                                 starting_d4[1])/2, (starting_d4[2] + starting_d5[2] + starting_e4[2])/3]
        ...
        if start_nan_flag == False:
            starting_length = starting_bottom_left_vertex[1] - \
                starting_bottom_right_vertex[1]
            starting_width = starting_top_left_vertex[0] - \
                starting_bottom_left_vertex[0]
            ...
            if start_bottom_left_flag and start_bottom_right_flag:
                ...
                offset = starting_bottom_left_vertex[1] - \
                    starting_top_left_vertex[1]
                starting_length = starting_top_left_vertex[1] - \
                    starting_top_right_vertex[1] + (offset * 2)
            elif start_top_left_flag and start_top_right_flag and start_bottom_right_flag:
                offset = starting_top_right_vertex[1] - \
                    starting_bottom_right_vertex[1]
                starting_length = starting_top_left_vertex[1] - \
                    starting_top_right_vertex[1] + (offset * 2)
            elif start_top_left_flag and start_bottom_right_flag:
                starting_length = (
                    starting_top_left_vertex[1] - starting_bottom_right_vertex[1]) + 0.05
            elif start_top_right_flag and start_bottom_left_flag:
                starting_length = (
                    starting_bottom_left_vertex[1] - starting_top_right_vertex[1]) + 0.05

            if start_top_left_flag and start_bottom_left_flag:
                starting_width = starting_top_left_vertex[0] - \
                    starting_bottom_left_vertex[0]
            elif start_top_right_flag and start_bottom_right_flag:
                starting_width = starting_top_right_vertex[0] - \
                    starting_bottom_right_vertex[0]
        ...
        if start_bottom_left_flag:
            starting_R, starting_reference_normal = self.get_rot_matrix(
                starting_pcl, starting_normals, starting_bottom_left_vertex)
            ...
            starting_R, starting_reference_normal = self.get_rot_matrix(
                starting_pcl, starting_normals, starting_bottom_right_vertex)

        # Create a bounding box around the chessboard
        starting_bounding_box_chessboard, starting_frame = self.create_bounding_box(
            starting_pcl, starting_length, starting_width, starting_R, starting_center, starting_reference_normal, 0.25, 0.15, 0)
        ...
        starting_chessboard = self.crop(
            starting_pcl, starting_bounding_box_chessboard)
        # Compute the mean distance between two consecutive squares in the y direction and in the x direction
        sum_width = 0
        sum_length = 0
        for i in range(1, 7):
            sum_width = sum_width + \
                abs(starting_squares_pointcloud.points[self.rows_indexes['row8'][i]][0] -
                    starting_squares_pointcloud.points[self.rows_indexes['row8'][i-1]][0])
            sum_width = sum_width + \
                abs(starting_squares_pointcloud.points[self.rows_indexes['row5'][i]][0] -
                    starting_squares_pointcloud.points[self.rows_indexes['row5'][i-1]][0])
            sum_width = sum_width + \
                abs(starting_squares_pointcloud.points[self.rows_indexes['row1'][i]][0] -
                    starting_squares_pointcloud.points[self.rows_indexes['row1'][i-1]][0])
            sum_length = sum_length + abs(starting_squares_pointcloud.points[self.columns_indexes['columnA'][i]]
                                          [1] - starting_squares_pointcloud.points[self.columns_indexes['columnA'][i-1]][1])
            sum_length = sum_length + abs(starting_squares_pointcloud.points[self.columns_indexes['columnD'][i]]
                                          [1] - starting_squares_pointcloud.points[self.columns_indexes['columnD'][i-1]][1])
            sum_length = sum_length + abs(starting_squares_pointcloud.points[self.columns_indexes['columnH'][i]]
                                          [1] - starting_squares_pointcloud.points[self.columns_indexes['columnH'][i-1]][1])
        ...
        for squares in starting_squares_pointcloud.points:
            bounding_box_square, frame = self.create_bounding_box(
                starting_chessboard, mean_y_dist, mean_x_dist, starting_R, squares, starting_reference_normal, 0.05, 0.02, 0.035)
        ...
        for occupied in self.currently_occupied:
            square = self.crop(
                starting_chessboard, squares_bound_boxes[self.squares_to_index[occupied]])
        ...
        mean_points_occupied = mean_points_occupied // len(
            self.currently_occupied)
        ...
        for not_occupied in self.currently_not_occupied:
            square = self.crop(
                starting_chessboard, squares_bound_boxes[self.squares_to_index[not_occupied]])
            points_number = len(np.asarray(square.points))
            mean_points_not_occupied = mean_points_not_occupied + points_number
        mean_points_not_occupied = mean_points_not_occupied // len(
            self.currently_not_occupied)
    ...
    def look_for_pieces(self, pcl, chessboard, squares_boundboxes, threshold, color_threshold, is_color_over):
        ...
        opponent_move = OccupancyChecke()
        self.possible_capture, self.possible_start_square, self.possible_end_square, self.possible_castle, self.possible_enpasent, self.possible_promotion = opponent_move.opponent_move(self.rgb_subscriber_state1,self.rgb_subscriber_state2,self.depth_subscriber_state2,True)
        ...
        if len(self.possible_castle) != 0:
            ...
            for i in self.possible_enpasent:
                if self.opposite_color == 'white':
                    if i[1] == '5' and self.dir_live_chessboard_situation[i][1] ==self.opposite_color:
                        move_square == i
                    ...
                    if i[1] == '4' and self.dir_live_chessboard_situation[i][1] ==self.opposite_color:
                        move_square == i
                        moved_piece = self.dir_live_chessboard_situation[i][0]   
                    elif i[i] =='3':
                        ending_square = i
        ...
        #         square = self.crop(
        #             pcl, squares_boundboxes[self.squares_to_index[occupied]])
        ...
        #             if index in values:
        #                 column = key
        #         # * 0.5): #If the points over the square are less than a threshold, it means that the piece have been moved. take the 50% of the threshold to exclude the pieces covered by pieces in front of them.
        #         if points_number < (threshold * 0.7):
        ...
        #         print('Less points piece: '+str(less_points_piece))
            ...
            if count_empty > 1:
                ...
                if not castle:
                    for piece in check_pieces:
                        if self.pieces_coordinates[piece][1]['name'] == 'pawn':
                            ending_square, en_passant, en_passant_square, promotion = self.pawn_checking(
                                check_squares[conteggio], chessboard, threshold, squares_boundboxes, color_threshold, is_color_over)
                        elif self.pieces_coordinates[piece][1]['name'] == 'knight':
                            ending_square, en_passant, en_passant_square, promotion = self.knight_checking(
                                check_squares[conteggio], chessboard, threshold, squares_boundboxes, color_threshold, is_color_over)
                        elif self.pieces_coordinates[piece][1]['name'] == 'bishop':
                            ending_square, en_passant, en_passant_square, promotion = self.bishop_checking(
                                check_squares[conteggio], chessboard, threshold, squares_boundboxes, color_threshold, is_color_over)
                        elif self.pieces_coordinates[piece][1]['name'] == 'rook':
                            ending_square, en_passant, en_passant_square, promotion = self.rook_checking(
                                check_squares[conteggio], chessboard, threshold, squares_boundboxes, color_threshold, is_color_over)
                        elif self.pieces_coordinates[piece][1]['name'] == 'queen':
                            ending_square, en_passant, en_passant_square, promotion = self.queen_checking(
                                check_squares[conteggio], chessboard, threshold, squares_boundboxes, color_threshold, is_color_over)
                        elif self.pieces_coordinates[piece][1]['name'] == 'king':
                            ending_square, en_passant, en_passant_square, promotion = self.king_checking(
                                check_squares[conteggio], chessboard, threshold, squares_boundboxes, color_threshold, is_color_over)
                        conteggio += 1
                    ...
                    for i in range(0, len(check_ending_square)):
                        ...
                        # Controllo per evitare i pedoni coperti da altri pezzi
                    ...
                    if count_occupied == 0:
                        rospy.logwarn(
                            'The opponent move has not been identified')
                ...
                if self.pieces_coordinates[moved_piece][1]['name'] == 'pawn':
                    ...
                    square_box = self.crop(
                        chessboard, squares_boundboxes[next_square])
                    points_number = len(np.asarray(square_box.points))
                    if points_number > threshold:  # The square is occupied.
                        # Since the square is occupied, the pawn could be hidden by another piece.
                        moved_piece = less_points_piece
        ...
        if not castle and count_empty < 2:  # and checking_flag: #AGGIUNTA LA PARTE DOPO L'AND
            if self.pieces_coordinates[moved_piece][1]['name'] == 'pawn':
                ending_square, en_passant, en_passant_square, promotion = self.pawn_checking(
                    move_square, chessboard, threshold, squares_boundboxes, color_threshold, is_color_over)
    ...
    def moved_piece_identification(self, pcl):
        t_crop = time()
        # To use if I'm using the original points in the eyes reference frame.
        squares_matrix = self.get_points_in_matrix_from_pointstamped(
            self.squares_centers)
        # To use if I'm usign the original points in the eyes reference frame.
        vertices_matrix = self.get_points_in_matrix_from_pointstamped(
            self.vertices)
        # Creation of a red PointCloud of the squares centers computed segmenting the chessboard.
        squares_pointcloud = self.create_pointcloud(squares_matrix, [1, 0, 0])
        # Creation of a green PointCloud of the vertices computed segmenting the chessboard.
        vertices_pointcloud = self.create_pointcloud(
            vertices_matrix, [0, 1, 0])
        ...
        if self.verbose:
            rospy.loginfo('Normals estimation took ' +
                          str(time() - t_norm) + 's')
        ...
        if nan_flag == False:
            ...
            if bottom_left_flag and bottom_right_flag:
                ...
                length = top_left_vertex[1] - \
                    top_right_vertex[1] + (offset * 2)
        ...
        # Rotation matrix
        if bottom_left_flag:
            R, reference_normal = self.get_rot_matrix(
                pcl, normals, bottom_left_vertex)
        elif bottom_right_flag:
            # starting_bottom_left_vertex = starting_bottom_right_vertex
            # starting_bottom_left_vertex[1] = starting_bottom_right_vertex[1] + starting_length
            R, reference_normal = self.get_rot_matrix(
                pcl, normals, bottom_right_vertex)

        # Create a bounding box around the chessboard
        bounding_box_chessboard, frame = self.create_bounding_box(
            pcl, length, width, R, center, reference_normal, 0.25, 0.15, 0)
        ...
        for i in range(1, 7):
            sum_width = sum_width + \
                abs(squares_pointcloud.points[self.rows_indexes['row8'][i]][0] -
                    squares_pointcloud.points[self.rows_indexes['row8'][i-1]][0])
            sum_width = sum_width + \
                abs(squares_pointcloud.points[self.rows_indexes['row5'][i]][0] -
                    squares_pointcloud.points[self.rows_indexes['row5'][i-1]][0])
            sum_width = sum_width + \
                abs(squares_pointcloud.points[self.rows_indexes['row1'][i]][0] -
                    squares_pointcloud.points[self.rows_indexes['row1'][i-1]][0])
            sum_length = sum_length + \
                abs(squares_pointcloud.points[self.columns_indexes['columnA'][i]][1] -
                    squares_pointcloud.points[self.columns_indexes['columnA'][i-1]][1])
            sum_length = sum_length + \
                abs(squares_pointcloud.points[self.columns_indexes['columnD'][i]][1] -
                    squares_pointcloud.points[self.columns_indexes['columnD'][i-1]][1])
            sum_length = sum_length + \
                abs(squares_pointcloud.points[self.columns_indexes['columnH'][i]][1] -
                    squares_pointcloud.points[self.columns_indexes['columnH'][i-1]][1])
        ...
        for squares in squares_pointcloud.points:
            bounding_box_square, frame = self.create_bounding_box(
                chessboard, mean_y_dist, mean_x_dist, R, squares, reference_normal, 0.05, 0.02, 0.035)
            squares_bound_boxes.append(bounding_box_square)
            squares_frames.append(frame)
        ...
        if self.verbose:
            rospy.loginfo('Cropping chessboard pointcloud took' +
                          str(time() - t_crop) + 's.')

        # Look for pieces
        # o3d.visualization.draw_geometries([pcl])
        moved_piece, starting_square, ending_square = self.look_for_pieces(
            pcl, chessboard, squares_bound_boxes, occupied_threshold, color_threshold, is_color_over)
        print('MOVED PIECE: ' + str(moved_piece))
    ...
    def identify_moved_piece(self, data):
        ...
        if data:
            ...
            moved_piece, start_square, end_square = self.moved_piece_identification(
                self.pcd)
    ...
    def CallbackState(self, data):
        ...
        if data.data == 14:
            self.rgb_subscriber_state1 = message_filters.Subscriber(
            '/xtion/rgb/image_rect_color/compressed', CompressedImage)
        if data.data == 15 and analysis_not_done_yet:
            ...
            # Take as threshold, the 75% of the mean of points of the occupied squares and the not occupied ones.  * 0.75
            occupied_threshold = (
                (mean_points_occupied + mean_points_not_occupied) // 2) * 0.75
            color_threshold = [((color_RGB[0] + opponent_color_RGB[0]) / 2), ((
                color_RGB[1] + opponent_color_RGB[1]) / 2), ((color_RGB[2] + opponent_color_RGB[2]) / 2)]
            ...
            # DA VERIFICARE SE NON LO PUBBLICA TROPPO PRESTO
