...
class MoveDetection:
    ...
    def detect_square_change(self, previous, current,tiago_color, debug=False):
        ...
        if __name__ == '__main__':
            ...
            cv2.imshow('Image previous',previous)
            cv2.imshow('Image current',current)
            cv2.imshow(
    ...
    def possible_squares_start_end(self, chessboardedges_with_out_borders, state1_image, state2_image, tiago_color,debug=0):
        """_summary_
...
if __name__ == '__main__':
    ...
    #     img_current = cv2.imread(img_path)
        ...
        img_current = cv2.imread(img_path_after)

        T = m_d.possible_squares_start_end(chessboard_square_corners, img_previous, img_current, tiago_color='white')
