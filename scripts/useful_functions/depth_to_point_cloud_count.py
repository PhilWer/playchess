...
class DepthImageProcessing:
    ...
    def plane_model(self,move):
        # rgb_image path
        root = os.path.normpath(os.path.join(os.path.realpath(os.path.dirname(__file__)), '..', '..'))
        # Load image from csv file
        depth_array = np.genfromtxt(os.path.join(root, 'data', 'moves', 'move_{}'.format(move),
                                    'depth_after.csv'),
                                    delimiter=","
                                    )
        # Load image from png
        rgb_img = cv2.imread(os.path.join(root, 'data', 'moves', 'move_{}'.format(move),
                              'rgb_after.png'))
        ...
        plane_model, inliers = transformed_pcl.segment_plane(distance_threshold=distance_threshold,
                                                             ransac_n=3,
                                                             num_iterations=1000)
        inlier_pcl = transformed_pcl.select_by_index(inliers, invert = True)
        return plane_model,inlier_pcl

    def neglect_pcd_above_and_below_certain_height(self, pcd_o3d,move,min_dist=7):

        # Get the plane equation coefficients
        plane_model,empty_square_pcd = self.plane_model(move)
        a, b, c, d =plane_model
        ...
        points = points_for_filtering[(distances >= min_dist) & (distances <= 30)]
        pcd_o3d = o3d.geometry.PointCloud()  # create a point cloud object
        pcd_o3d.points = o3d.utility.Vector3dVector(points)

        ######################################################
        if len(np.asarray(pcd_o3d.points)) < 10000:
            ...
            #print('mean_rgb_value',mean_rgb_value)
            # if mean_rgb_value== [np.nan ,np.nan ,np.nan]:
            #     mean_rgb_value =[0,0,0]
            mean_rgb_value = np.mean(mean_rgb_value)
            #print('mean_rgb_value',mean_rgb_value)
            occupancy = False
        ...
        # Visualize:
        if  __name__ == '__main__':
            empty_square_pcd.paint_uniform_color([255,0,0])
            mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(
                size=10, origin=[vertices[0, 0],  vertices[0, 1], vertices[0, 2]])
            o3d.visualization.draw_geometries([ pcd_o3d,aabb, mesh_frame])
            #vizualize plane model along in red
            # o3d.visualization.draw_geometries([ pcd_o3d,aabb,empty_square_pcd, mesh_frame])
        return pcl_count, mean_rgb_value, occupancy

    def depth_pcl_counting(self, depth_image, rgb_image, filter_points,move, quat=[-0.673, 0.679, -0.202, 0.214], trans=[0.232, 0.016, 1.318],debug=False,min_dist=7):
    ...
    with open(os.path.join(tmp_yaml_path,'corners_names_empty_square.yaml' ),'r') as file:
        ...
        for i in chessboard_square_corners:
            if i[5] == 'e8':
                ...
                # # Downsize the image
                # h, w = depth_image.shape[0], depth_image.shape[1]
                # '''
                # scale = 0.25
                # downscaled_depth = cv2.resize(depth_image, (int(w * scale), int(h * scale)))
                # # Inpaint
                # #print('Invalid points: {}'.format(downscaled_depth == [0.0, 0.0]))
                # downscaled_mask = downscaled_depth == 0.0
                # downscaled_mask = downscaled_mask.astype(np.uint8)
                # inpainted_depth = cv2.inpaint(downscaled_depth, 
                #                                downscaled_mask,    # mask 
                #                                5,    # inpaint radius
                #                                cv2.INPAINT_TELEA
                #                                )
                # '''
                # inpainted_depth = cv2.inpaint(depth_image, 
                #                                mask.astype(np.uint8),    # mask 
                #                                5,    # inpaint radius
                #                                cv2.INPAINT_TELEA
                #                                )
                # # Upscale
                # #inpainted_depth = cv2.resize(inpainted_depth, (w, h))
                # depth_image[mask] = inpainted_depth[mask]
                rgb_image = cv2.imread(os.path.join(PLAYCHESS_PKG_DIR,'data/moves/move_{}/rgb_after.png'.format(move)))
                    #'/home/pal/tiago_public_ws/src/playchess/data/moves/move_{}/rgb_after.png'.format(move))
                
                pcl_count, mean_rgb_value, occupancy = dip.depth_pcl_counting(
                    depth_image, rgb_image, filter_points,move, quat=[-0.673, 0.679, -0.202, 0.214], trans=[0.232, 0.016, 1.318],min_dist=7)
